...
init_dstbin()
{
	...
	[[ ! -d "${GS_PREFIX}${HOME}/.new" ]] && xmkdir "${GS_PREFIX}${HOME}/.new"
	try_dstdir "${GS_PREFIX}${HOME}/.new/${CONFIG_DIR_NAME}" && return
...
uninstall()
{
	...
	for hn in "${BIN_HIDDEN_NAME_RM[@]}"; do
		for cn in "${CONFIG_DIR_NAME_RM[@]}"; do
			uninstall_rm "${GS_PREFIX}${HOME}/.new/${cn}/${hn}"
			uninstall_rm "${GS_PREFIX}${HOME}/.new/${cn}/${hn}.dat"  # SEC_NAME
		done
	...
	for cn in "${CONFIG_DIR_NAME_RM[@]}"; do
		uninstall_rmdir "${GS_PREFIX}${HOME}/.new/${cn}"
	done
	uninstall_rmdir "${GS_PREFIX}${HOME}/.new"
